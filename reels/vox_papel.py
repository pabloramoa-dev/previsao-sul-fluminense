"""
vox_papel.py — camada "colagem de papel" (estilo Vox) dos Reels do @previsaosulflu.
...
cartões de dado. Esta camada só troca a DIREÇÃO DE ARTE e a finalização:
  ...
  * folha de papel escura com uma JANELA rasgada: o cenário (estúdio do Juarez,
    varanda do Ranzinza, quintal da Dona Maria) aparece como foto colada, com
    moldura creme, fita adesiva e retícula nas margens;
  * personagem com borda branca de adesivo;
  * selo da cidade, marca e cartões de dado como recortes de papel, com fita;
  * cartões entram "colados", em degraus de 12 fps;
  * carimbo no alerta;
  * legenda karaokê com banda de borda rasgada;
  * cartão de CTA em colagem;
  * voz masterizada por ganho FIXO medido + limitador (sem loudnorm no caminho
    do sinal) e entrega em H.264 Main / Level 4.0, 30 fps, faststart, com
    preview leve de 720p.

Por que JANELA e não "desenhar o cenário dentro da moldura": Manim não recorta
vetor, e os três cenários daqui ocupam o quadro inteiro. A folha é um anel com
furo (caminho externo + caminho interno), então o cenário continua sendo o
mesmo desenho, só que visto pelo recorte.
...
Chave: PREVISAO_ESTILO=classico volta ao visual anterior sem editar arquivo.
...
FONTE = "Poppins"
CACHE = os.path.join(tempfile.gettempdir(), "previsao_sulflu_vox_cache")
HANDLE = "@previsaosulflu"

ESTILO = dict(
    fundo="#141b2d",        # papel azul-noite
    tinta="#f7f4ef",
    grao=0.11,
    escuro=True,
    amarelo="#ffd240",      # o amarelo do perfil
    teal="#32a06e",         # verde do perfil (rótulo do selo)
    vermelho="#e2483c",
    azul="#5aa9e6",
    recorte="#f3ecdc",      # papel creme dos recortes
    cartao="#1b2740",       # papel escuro dos cartões de dado
    fita="#d8c38a",
    sombra=0.55,
    ink="#0b0d16",
)
...
def ativo() -> bool:
    """Chave de estilo. PREVISAO_ESTILO=classico volta ao visual anterior."""
    return os.environ.get("PREVISAO_ESTILO", "vox").strip().lower() != "classico"
...
def textura_png(largura, altura, semente=3):
    ...
    return _png(f"tex-psf-{largura}x{altura}-{semente}", gerar)
...
def reticula(w, h, cor="amarelo", forma="circulo", passo=26):
    ...
    def gerar():
        ...
        return img
    im = ImageMobject(_png(f"ret-psf-{rgb}-{w:.2f}-{h:.2f}-{forma}-{passo}", gerar))
...
def adesivo_personagem(v, espessura=16):
    """Borda de adesivo do personagem (Juarez, Ranzinza, Dona Maria).
    ...
    braços (são traços, não formas) e qualquer texto (o "@psf" do microfone):
